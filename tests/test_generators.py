...
@pytest.fixture
# Список исходных данных - пустые описания
def example_transactions_without_descriptions() -> list[dict]:
    return [
        {
            "id": 939719570,
            "state": "EXECUTED",
...
@pytest.fixture
# Список исходных данных c кривыми ключами
def example_transactions_keys_error() -> list[dict]:
    return [
        {
            "id": 939719570,
            "state": "EXECUTED",
            "date": "2018-06-30T02:08:58.425572",
            "operationAmount": {"amount": "9824.07", "KUR": {"name": "USD", "KOD": "USD"}},
            "D": "Перевод организации",
            "from": "Счет 75106830613657916952",
            "to": "Счет 11776614605963066702",
        },
        {
            ...
            "state": "EXECUTED",
            "date": "2019-04-04T23:20:05.206878",
            "operationAmount": {"amount": "79114.93", "KUR": {"name": "USD", "KOD": "USD"}},
            "D": "Перевод со счета на счет",
            "from": "Счет 19708645243227258542",
            "to": "Счет 75651667383060284188",
        },
        {
            ...
            "state": "EXECUTED",
            "date": "2019-03-23T01:09:46.296404",
            "operationAmount": {"amount": "43318.34", "KUR": {"name": "руб.", "KOD": "RUB"}},
            "D": "Перевод со счета на счет",
            "from": "Счет 44812258784861134719",
            "to": "Счет 74489636417521191160",
        },
        {
            ...
            "state": "EXECUTED",
            "date": "2018-08-19T04:27:37.904916",
            "operationAmount": {"amount": "56883.54", "KUR": {"name": "USD", "KOD": "USD"}},
            "D": "Перевод с карты на карту",
            "from": "Visa Classic 6831982476737658",
            "to": "Visa Platinum 8990922113665229",
        },
        {
            ...
            "state": "CANCELED",
            "date": "2018-09-12T21:27:25.241689",
            "operationAmount": {"amount": "67314.70", "KUR": {"name": "руб.", "code": "RUB"}},
            "D": "Перевод организации",
            "from": "Visa Platinum 1246377376343588",
            "to": "Счет 14211924144426031657",
        },
...
@pytest.fixture
# Отсутствуют значения ["operationAmount"]["currency"]["code"] в словарях
def example_transactions_without_codes() -> list[dict]:
    return [
        {
            "id": 939719570,
            "state": "EXECUTED",
        ...
        },
        {
            ...
            "state": "EXECUTED",
            "date": "2019-03-23T01:09:46.296404",
            "operationAmount": {"amount": "43318.34", "currency": {"name": "руб.", "code": ""}},
            "description": "Перевод со счета на счет",
            "from": "Счет 44812258784861134719",
            "to": "Счет 74489636417521191160",
        },
...
@pytest.mark.parametrize(
    ...
    [
        ("USD", 3),  # где 3 — ожидаемое количество транзакций в USD
        ("RUB", 2),
    ],
)
def test_filter_by_currency_counts(example_transactions: list[dict], input_data: str, expected_output: int) -> None:
    result = list(filter_by_currency(example_transactions, input_data))
...
def test_filter_by_currency_not_codes(example_transactions_without_codes: list[dict]) -> None:
    """Пример работы функции если в списке словарей пустые значения содов операций"""
    generator_funk = filter_by_currency(example_transactions_without_codes, "USD")
    assert next(generator_funk) == "No transactions"


def test_filter_by_currency_not_keys(example_transactions_keys_error: list[dict]) -> None:
    """Пример работы функции если в нет ключа x["operationAmount"]["currency"]["code"]"""
    generator_funk = filter_by_currency(example_transactions_keys_error, "USD")
    assert next(generator_funk) == "key not found"
...
def test_transaction_not_descriptions(example_transactions_without_descriptions: list[dict]) -> None:
    """Пример работы функции transaction_descriptions если отсутствуют описания в структуре данных"""
    gen_descriptions = transaction_descriptions(example_transactions_without_descriptions)
    assert next(gen_descriptions) == "No transactions"


def test_transaction_descriptions_key_error(example_transactions_keys_error: list[dict]) -> None:
    """Пример работы функции transaction_descriptions если нет ключа ["description"]"""
    gen_descriptions = transaction_descriptions(example_transactions_keys_error)
    assert next(gen_descriptions) == "key description not found"
